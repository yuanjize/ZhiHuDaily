...
public class NewsContentPageAdapter extends PagerAdapter {
    ...
    private void getNewsContent(int storyId, final WebView webView) {
        ...
        RxUtils.getNewsContent(String.valueOf(storyId)).subscribe(new BaseSubscribe<NewsContent>(URLs.NEWS_CONTENT, NewsContent.class.getComponentType()) {
            @Override
            protected void handleMessage(NewsContent newsContent) {
                WebSettings settings = webView.getSettings();
                settings.setJavaScriptEnabled(true);
                settings.setLayoutAlgorithm(WebSettings.LayoutAlgorithm.SINGLE_COLUMN);
                String page = "<html><head>" +
                        "<link rel='stylesheet' type='text/css' href='" +
                        newsContent.getCss() + "'/>" + "<style>img{display: inline;height: auto;max-width: 100%;}</style>" +
                        "</head>" + "<body>" + newsContent.getBody() + "</body></html>";
                webView.loadData(page, "text/html;charset=utf-8", "utf-8");
            }
        });

//
//        NetWorkUtil.request(NewsService.class, "getNewsContent", new BaseRetrofitCallBack<NewsContent>(URLs.NEWS_CONTENT, NewsContent.class) {
//            @Override
